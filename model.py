...
def deepnn(x):
  ...
  return tf.add(tf.matmul(h_fc2, W_fc3), b_fc3)
  ...
  with tf.Session() as sess:
    ...
    for step in range(max_train_steps):
      ...
      if step % 100 == 0:
        ...
        print(f'step {step}, training accuracy {train_accuracy}')
      ...
      if step % 1000 == 0:
        ...
        print(f'*Test accuracy {accuracy.eval(feed_dict={x: fer2013.validation.images, y_: fer2013.validation.labels})}')
...
def image_to_tensor(image):
  return np.asarray(image).reshape(-1, 2304) * 1 / 255.0
